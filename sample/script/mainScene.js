...
module.exports = function() {
  ...
  scene.loaded.add(function() {
    ...
    var b2 = new Box2D.Box2D(worldOption);
    ...
    // 地面エンティティの性質を定義
    var floorDef = b2.createFixtureDef({
      density: 1.0, // 密度
      friction: 0.5, // 摩擦係数
    ...
    // 地面エンティティを静的物体化
    var staticDef = b2.createBodyDef({
      type: Box2D.BodyType.Static
    });

    // Box2Dに地面エンティティを追加
    var floorBody = b2.createBody(floor, staticDef, floorDef);
    ...
    // エンティティの性質を定義
    var entityDef = b2.createFixtureDef({
      density: 1.0, // 密度
      friction: 0.5, // 摩擦係数
      restitution: 0.3 // 反発係数
    });
    ...
    // 動的物体化
    var dynamicDef = b2.createBodyDef({
    ...
    var rectBody1 = b2.createBody(rect1, dynamicDef, entityDef);

    // rect2エンティティを四角に設定
    entityDef.shape = b2.createRectShape(rect2.width, rect2.height);
    // rect2エンティティをBox2Dに追加
    var rectBody2 = b2.createBody(rect2, dynamicDef, entityDef);

    // サッカーボールエンティティを円に設定
    entityDef.shape = b2.createCircleShape(soccer.width);
    // サッカーボールエンティティをBox2Dに追加
    var soccerBody = b2.createBody(soccer, dynamicDef, entityDef);
    ...
    // 五角形エンティティをBox2Dに追加
    var pentagonBody = b2.createBody(pentagon, dynamicDef, entityDef);
